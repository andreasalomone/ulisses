...
export default async function MinisteroTurismoPage() {
    const t = await getTranslations('specialStories');
    const ts = await getTranslations('specialStories.ministero');
                            ...
                            <div className="bg-muted/30 rounded-3xl p-6 border border-border/50 backdrop-blur-sm">
                                ...
                                <ul className="space-y-4">
                                    <li className="flex justify-between items-center border-b border-border/40 pb-2 last:border-0 last:pb-0">
                                        <span className="text-sm">{ts('stats.occupancy')}</span>
                                        <span className="font-bold text-foreground text-right text-xs">{ts('stats.realtime')}</span>
                                    </li>
                                    <li className="flex justify-between items-center border-b border-border/40 pb-2 last:border-0 last:pb-0">
                                        <span className="text-sm">{ts('stats.tracking')}</span>
                                        <span className="font-bold text-foreground text-right text-xs">{ts('stats.assets')}</span>
                                    </li>
                                    <li className="flex justify-between items-center border-b border-border/40 pb-2 last:border-0 last:pb-0">
                                        <span className="text-sm">{ts('stats.analysis')}</span>
                                        <span className="font-bold text-foreground text-right text-xs">{ts('stats.history')}</span>
                                    </li>
                                </ul>
                            </div>
