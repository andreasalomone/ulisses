...
import { CheckCircle2, LayoutDashboard, Bell, Activity, Layers } from "lucide-react";
...
interface SmartParkingExplorerProps {
    segments: Persona[];
    useCases: UseCase[];
    mapping: Record<string, string[]>;
}

export function SmartParkingExplorer({
    segments,
    useCases,
    mapping,
}: SmartParkingExplorerProps) {
    ...
    return (
        <div className="flex flex-col gap-12">
            {/* Persona Selector - Desktop: Side-by-side, Mobile: Stack */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {segments.map((segment) => (
                    <button
                        ...
                        className={cn(
                            "relative p-8 rounded-[2.5rem] border text-left transition-all duration-500 group",
            ...
            {/* Use Case Grid */}
            <div className="relative p-8 md:p-12 rounded-[3.5rem] bg-muted/30 border border-primary/5 min-h-[400px]">
